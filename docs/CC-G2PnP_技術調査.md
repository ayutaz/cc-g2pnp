...
```
LLM → BPEトークンID [B, T]
         ↓ Embedding(65024, 512) + Repeat Upsample(×8)
       隠れ表現 [B, T×8, 512]
...
3. **Convolution Module** (causal depthwise conv)
   - LayerNorm → Pointwise Conv(512→1024) → GLU → Depthwise Conv1D(kernel=31) → BatchNorm → Swish → Pointwise Conv(512→512) → Dropout
...
| Conformer層数 | 8 | 論文明記 |
| 隠れ次元 | 512 | 論文明記 |
| アテンションヘッド数 | 8 | 論文未記載、Conformer標準は4だが、d_model=512では8が一般的 |
| FFN expansion factor | 4 | Conformer標準 |
| 畳み込みカーネルサイズ | 31 | 論文未記載、Conformer標準 |
| Convolution expansion factor | 2 | Conformer標準（GLU前の拡張） |
| BPE語彙サイズ | 65,024 | CALM2-7B-Chat |
| アップサンプリング係数 | 8 | 論文明記 |
| 過去コンテキスト P | 10 | 全モデル共通 |
| 中間CTC適用層 | 2, 4, 6 | 論文明記 |
| 中間CTC損失重み | 1/3 | 論文明記 |
...
- **ベースモデル**: Llamaアーキテクチャ（標準の32,000語彙から65,024に拡張）
...
print(tokenizer.vocab_size)  # 65024
```
...
既存フレームワークを**参考**にしつつ、**PyTorchベースでスクラッチ実装**する:
- NeMoからchunk-aware streamingとキャッシュ管理パターンを参考
- ESPnetからSelf-conditioned CTCの実装を参考
- SpeechBrainからDynamic Chunk Trainingのパターンを参考
- 外部フレームワークへの依存を最小化し、論文仕様を忠実に再現
...
- **CC-G2PnPのコードは未公開**（2026年2月時点、提出直後のため）
- **Dict-DNN韻律予測モデルも未公開**（Park et al., Interspeech 2022）
- **6D-Eval評価データセットも未公開**
...
ID 1-~100:  音素（カタカナ表記）
  ...
  - わ行(2): ワ,ヲ
  - 特殊(3): ン,ッ,ー
  - 外来語等: ファ,フィ,フェ,フォ,ティ,ディ,デュ,トゥ,ドゥ,
             ウィ,ウェ,ウォ,ヴァ,ヴィ,ヴ,ヴェ,ヴォ 等
ID ~101:    * (アクセント核)
ID ~102:    / (アクセント句境界)
ID ~103:    # (イントネーション句境界)
ID ~104:    <unk>
...
| トークナイザ | CALM2-7B-Chat BPE（語彙65,024） |
| アップサンプリング係数 | 8 |
...
datasets>=2.14.0            # ReazonSpeechデータセット

# Japanese NLP
fugashi[unidic]             # MeCab + UniDic (形態素解析)
pyopenjtalk>=0.3.0          # G2P + Full-context labels
...
| リスク | 影響度 | 対策 |
|---|---|---|
| Dict-DNN韻律予測モデルが入手不可 | **高** | pyopenjtalkのfull-context label解析で代替（ttslearnのpp_symbols関数を参考）。段階的にBERTベース韻律予測を学習 |
...
| pyopenjtalkのインストール問題 | 中 | Cython互換性、CMake要件に注意。Windows: MSVC必要。`--no-build-isolation` オプションの活用 |
| ReazonSpeechデータ量の不一致 | 低 | 論文の14,960,911件は`all`設定からフィルタリングされた可能性あり。フィルタリング条件の推定が必要 |
| CALM2トークナイザの語彙サイズ不一致の可能性 | 低 | 65,024であることを確認済み |
| Conformerのアテンションヘッド数・カーネルサイズが未記載 | 低 | Conformerの標準値（heads=8, kernel=31）で開始し、必要に応じて調整 |
