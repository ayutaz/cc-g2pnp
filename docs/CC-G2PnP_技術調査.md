...
| pyopenjtalkのインストール問題 | 中 | `pyopenjtalk-plus>=0.4.1`（Python 3.13 Windows対応フォーク）を使用。API互換 |
| ReazonSpeechデータ量の不一致 | 低 | 論文の14,960,911件は`all`設定からフィルタリングされた可能性あり。フィルタリング条件の推定が必要 |
| CALM2トークナイザの語彙サイズ | — | **65,000** であることを確認済み（論文記載の65,024とは異なる） |
